...
class ViewController: UIViewController {
    ...
    @IBAction func pressFavChannel(sender: UISegmentedControl) {
        let name: String = sender.titleForSegmentAtIndex(sender.selectedSegmentIndex)!
        
        if(name == "ABC"){
            channelValue.text = "04"
        }
        if(name == "NBC"){
            channelValue.text = "11"
        }
        if(name == "CBS"){
            channelValue.text = "05"
        }
        if(name == "FOX"){
            channelValue.text = "29"
        }
    }
